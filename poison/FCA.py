...
class PoisonAttack:
    ...
    def fca_forward(self,old_poison,target_tensor,feature_extractor,isolate_model):
        poison = old_poison.clone().detach().requires_grad_(True)
        target = target_tensor.clone().detach().requires_grad_(True)
        poison_feature = feature_extractor(poison)
    ...
    def poison(self):
        ...
        isolate_model.train()
        feature_extractor = FeatureExtractor(isolate_model,self.feature_layer)
        ...
        for image_tensor in self.base_list:
            ...
            # isolate_model.to(self.device)
            # isolate_model.train()
            # isolate_fe = FeatureExtractor(isolate_model,self.feature_layer)

            for i in trange(self.max_iteration,desc="FCAing!!!"):
                new_poison = self.fca_forward(old_poison_batch,target_image_batch,feature_extractor,isolate_model)
                ...
                if(ref_change_value < self.stopping_tol):
                    ...
                    break
                new_poison_feature = feature_extractor(new_poison)
