    ...
    for step, (data, targets) in enumerate(train_loader):
        ...
        for data_chunk, target_chunk in zip(data_chunks, target_chunks):
            ...
            if config.device != 'cpu' and config.train.use_apex:
                with apex.amp.scale_loss(loss, optimizer) as scaled_loss: #type: ignore
        ...
        if config.train.gradient_clip > 0:
            if config.device != 'cpu' and config.train.use_apex:
                torch.nn.utils.clip_grad_norm_(
                    apex.amp.master_params(optimizer), #type: ignore
        ...
        acc1, acc5 = compute_accuracy(config,
                                      ...
                                      targets,
                                      augmentation=True,
                                      topk=(1, 5))   # change the top k according to the class num of custom dataset
...
def validate(epoch, config, model, loss_func, val_loader, logger,
    ...
    with torch.no_grad():
        for step, (data, targets) in enumerate(val_loader):
            ...
            # change the k index according to your class num of custom dataset
            acc1, acc5 = compute_accuracy(config,
                                          outputs,
                                          targets,
                                          augmentation=False,
...
def main():
    ...
    if config.device != 'cpu' and config.train.use_apex:
        model, optimizer = apex.amp.initialize( #type: ignore
